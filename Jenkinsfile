...
"""
  ...
  environment {
      IMAGE_REPO = "hemanthhr/testdummy"
      // Instead of nkhare, use your Dockerhub name
  }
  stages {
    stage('Build') {
      environment {
    ...
    stage('Deploy') {
      environment {
        ...
        GIT_REPO_URL = "github.com/apphr/rsvpapp-helm-cicd.git"
        GIT_REPO_EMAIL = 'admin@appreview.mobi'
